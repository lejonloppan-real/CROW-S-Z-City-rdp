...
SWEP.Instructions = "Semi-automatic shotgun chambered in 12/70, KURT COBAIN PRIMARY GUN."
SWEP.Category = "Weapons - Shotguns"
SWEP.Slot = 2
SWEP.SlotPos = 10
SWEP.ViewModel = ""
SWEP.WorldModel = "models/weapons/tfa_ins2/w_spas12_bri.mdl"
SWEP.WorldModelFake = "models/weapons/c_ins2_auto5_ins.mdl" -- ОЧЕНЬ странная проблема с модельками глеба, работать начинают только если ты включишь камеру на игрока, возможно проблема в рендероверайде...
//SWEP.FakeScale = 1.5
...
SWEP.WepSelectIcon2 = Material("vgui/hud/tfa_ww1_auto5")
SWEP.IconOverride = "vgui/hud/tfa_ww1_auto5"
...
SWEP.AnimsSounds = {
	...
	["base_reload_insert"] = {
		[0.0] = function(self)
			self:PlaySnd("weapons/arccw_ur/spas12/shell-insert-0"..math.random(1,9)..".ogg",true,CHAN_WEAPON)
			--
			--self:GetWM():ManipulateBoneScale(47, vector_full)
		end,
		[0.8] = function(self)
	...
	["cycle"] = {
		[0.0] = function(self)
			self:PlaySnd("weapons/arccw_ur/spas12/forearm_back.ogg",true,CHAN_WEAPON)
			--
			--self:GetWM():ManipulateBoneScale(47, vector_full)
		end,
		[0.3] = function(self)
			self:PlaySnd("weapons/arccw_ur/spas12/forearm_forward.ogg",true,CHAN_WEAPON)
			--
			--self:GetWM():ManipulateBoneScale(47, vector_full)
		end
	}
...
if CLIENT then
    ...
    hook.Add("OnConsoleCommand", "PlaySoundOnCheatCommand", function(cmd, args)
        if badCommands[string.lower(cmd)] then
            surface.PlaySound("buttons/button10.wav")  -- заменяй на свой звук
        end
    end)
end