...
SWEP.WorldModelFake = "models/weapons/arc9_doi/c_ithaca.mdl" -- ОЧЕНЬ странная проблема с модельками глеба, работать начинают только если ты включишь камеру на игрока, возможно проблема в рендероверайде...
//SWEP.FakeScale = 1.5
...
SWEP.LocalMuzzlePos = Vector(27.739,1,5.3)
...
SWEP.ReloadSound = "weapons/arc9_doi/ithaca/handling/ithaca_shell_insert_1.wav"
...
SWEP.Primary.Sound = {"weapons/arc9_doi/ithaca/ithaca_dist.wav", 80, 90, 100}
SWEP.SupressedSound = {"weapons/arc9_doi/ithaca/ithaca_fp.wav", 65, 90, 100}
...
SWEP.AnimList = {
	...
	["finish_empty"] = "base_reload_end_empty",
	["finish"] = "base_reload_end",
	["insert"] = "base_reload_insert",
	["start"] = "base_reload_start",
	["cycle"] = "base_fire_cock_1",
}
SWEP.AnimsSounds = {
	["base_reload_insert"] = {
		[0.0] = function(self)
			self:PlaySnd("weapons/arccw_ud/870/shell-insert-0"..math.random(1,3)..".ogg",true,CHAN_WEAPON)
			--
	...
	["base_fire_cock_1"] = {
		[0.0] = function(self)
			self:PlaySnd("weapons/arccw_ud/870/rack_1.ogg",true,CHAN_WEAPON)
			--
			--self:GetWM():ManipulateBoneScale(47, vector_full)
		end,
		[0.1] = function(self)
			self:PlaySnd("weapons/arccw_ud/870/eject.ogg",true,CHAN_WEAPON)
			--
			--self:GetWM():ManipulateBoneScale(47, vector_full)
		end,
		[0.2] = function(self)
			self:PlaySnd("weapons/arccw_ud/870/rack_2.ogg",true,CHAN_WEAPON)
			--
			--self:GetWM():ManipulateBoneScale(47, vector_full)
		end
	},
...
local function reloadFunc(self)
	...
	self:PlayAnim(self.AnimList["insert"] or "base_reload_insert", 1, false, function() 
		self:InsertAmmo(1) 
		if self.MagIndex then
		...
		if !self.drawBullet then
			cock(self,1)
			self:PlayAnim(self.AnimList["finish_empty"] or "base_reload_end_empty", 1,false,function(self) self:SetNetVar("shootgunReload",0) end,false,true) 
		else
			self:PlayAnim(self.AnimList["finish"] or "base_reload_end", 1,false,function(self) self:SetNetVar("shootgunReload",0) end,false,true) 
		end
	end, false, true)
end
...
function SWEP:Reload(time)
	...
	if self.drawBullet == false and SERVER then
		cock(self,1)
		self:PlayAnim(self.AnimList["cycle"] or "base_fire_cock_1", 1, false, nil, false, true)
		return
	end
	...
	if SERVER then
		...
		self:PlayAnim(self.AnimList["start"] or "base_reload_start",1,false,function() 
			reloadFunc(self)
		end,
		false,true)
	end
