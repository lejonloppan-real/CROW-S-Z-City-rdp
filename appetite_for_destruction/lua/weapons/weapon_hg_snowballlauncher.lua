...
SWEP.Spawnable = true
SWEP.AdminOnly = true
SWEP.PrintName = "Snowball Launcher"
SWEP.Author = "Degtyarev plant"
SWEP.Instructions = "SNOWBALL!!!"
...
SWEP.Primary.Ammo = "Snowballs"--rpg rounds
SWEP.Primary.Cone = 0
...
function SWEP:Shoot(override)
	...
	if SERVER then
		local projectile = ents.Create("ent_hg_snowball")
		projectile.owner = self:GetOwner()
		projectile:SetPos(pos + ang:Forward() * (self:GetOwner():GetVelocity():Length()/3) + ang:Right() * -6 + ang:Up() * 2)
		...
		projectile:Spawn()
		projectile:SetModel("models/zerochain/props_christmas/snowballswep/zck_w_snowballswep.mdl")
...
if CLIENT then
	...
	function SWEP:ReloadStart()
		...
		if self.MagIndex then
			...
			timer.Simple(self.ReloadTime - 1.7, function()
