...
function SWEP:CoreStep()
	...
	if SERVER then
		...
		if IsValid(actwep) and actwep ~= owner:GetWeapon("weapon_hands_sh") then
			local wclass = string.lower(actwep:GetClass() or "")
			local isPistol = actwep.IsPistolHoldType and actwep:IsPistolHoldType()
			...
			if self:KeyDown(IN_WALK) and self:KeyDown(IN_USE) then
				if not allowedWeapon then
					owner:SetNWFloat("human_shield_start", 0)
				else
				local tr = hg.eyeTrace(owner, 70)
				local ent = IsValid(tr.Entity) and (hg.RagdollOwner(tr.Entity) or tr.Entity) or nil
				if IsValid(ent) and ent:IsPlayer() and ent ~= owner then
					local dir = (owner:GetPos() - ent:GetPos()):GetNormalized()
					local backDot = ent:EyeAngles():Forward():Dot(dir)
					local isRagdolled = IsValid(ent.FakeRagdoll) or ent.GlideRagdoll
					local canTake = isRagdolled and owner:GetPos():Distance(ent:GetPos()) < 90
					if canTake then
						local start = owner:GetNWFloat("human_shield_start", 0)
						if start == 0 then
							owner:SetNWFloat("human_shield_start", CurTime())
						end
						if owner:GetNWFloat("human_shield_start", 0) > 0 and (CurTime() - owner:GetNWFloat("human_shield_start", 0)) >= 2 then
							owner:SetNWEntity("human_shield_victim", ent)
							ent:SetNWEntity("human_shield_attacker", owner)
							ent:SetNWBool("held_as_human_shield", true)
							if not ent:IsFrozen() then ent:Freeze(true) end
							owner:SetNWBool("human_shield_active", true)
						end
					else
						owner:SetNWFloat("human_shield_start", 0)
					end
			...
			if owner:GetNWBool("human_shield_active", false) then
				local victim = owner:GetNWEntity("human_shield_victim")
				if not IsValid(victim) or not victim:Alive() then
					owner:SetNWBool("human_shield_active", false)
					owner:SetNWEntity("human_shield_victim", nil)
					if IsValid(victim) then victim:SetNWBool("held_as_human_shield", false) victim:Freeze(false) end
				elseif not (self:KeyDown(IN_WALK) and self:KeyDown(IN_USE)) or self:KeyDown(IN_RELOAD) then
					victim:Freeze(false)
					owner:SetNWBool("human_shield_active", false)
					owner:SetNWEntity("human_shield_victim", nil)
					if IsValid(victim) then victim:SetNWBool("held_as_human_shield", false) end
				end
			end
		else
			owner:SetNWFloat("human_shield_start", 0)
